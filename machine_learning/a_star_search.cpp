...
namespace machine_learning {
  ...
  namespace aystar_search {
    ...
    template <size_t N = 3>
    class EightPuzzle {
      std::array<std::array<uint32_t, N>, N> board;  /// N x N array to store the current state of the Puzzle.

      std::vector<std::pair<int8_t, int8_t>> moves = {
        {0, 1}, {1, 0}, {0, -1}, {-1, 0}};  /// A helper array to evaluate the next state from current state;
      /**
   ...
   public:
       ...
       */
      explicit EightPuzzle(const std::array<std::array<uint32_t, N>, N> &init) : board(init) {}
       ...
       * @param A a reference of an EightPuzzle
       */
      EightPuzzle(const EightPuzzle<N> &&A) noexcept : board(std::move(A.board)) {}
      /**
      ...
      std::vector<EightPuzzle<N>> generate_possible_moves() {
        ...
        for (auto &move : moves) {
          if (in_range(zero_pos.first + move.first) && in_range(zero_pos.second + move.second)) {
            ...
            std::swap(new_config[zero_pos.first][zero_pos.second],
                      new_config[zero_pos.first + move.first][zero_pos.second + move.second]);
    ...
    class AyStarSearch {
      ...
      typedef struct Info {
        ...
        Info(const Puzzle &A, size_t h_value, size_t d)
          : state(std::make_shared<Puzzle>(A)), heuristic_value(h_value), depth(d) {}
         ...
         */
        Info(const Info &A)
          : state(std::make_shared<Puzzle>(A.state)), heuristic_value(A.heuristic_value), depth(A.depth) {}
   ...
   public:
       ...
       */
      std::vector<Puzzle> a_star_search(const std::function<uint32_t(const Puzzle &, const Puzzle &)> &dist,
                                        const uint32_t permissible_depth = 30) {
        MapOfPuzzleInfoWithPuzzleInfo parent_of;  /// Stores the parent of the states
        ...
        while (!open_list.empty()) {
          ...
          // state
          std::vector<Puzzle> total_possible_moves = current_state->state->generate_possible_moves();

          for (Puzzle &neighbor : total_possible_moves) {
            // calculate score of neighbors with respect to
            // current_state
            std::shared_ptr<Info> Neighbor =
              std::make_shared<Info>(neighbor, dist(neighbor, *(Final->state)), current_state->depth + 1U);
...
static void test() {
   ...
   */
  auto manhattan_distance = [](const machine_learning::aystar_search::EightPuzzle<> &first,
    ...
    for (size_t i = 0; i < first.get_size(); ++i) {
      for (size_t j = 0; j < first.get_size(); ++j) {
        ...
        if (m != first.get_size()) {
          ret += (std::max(m, i) - std::min(m, i)) + (std::max(n, j) - std::min(n, j));
        }
      }
    }
    return ret;
  ...
  machine_learning::aystar_search::AyStarSearch<machine_learning::aystar_search::EightPuzzle<3>> search(
    Puzzle, Ideal);  /// Search object
  ...
  search =
    machine_learning::aystar_search::AyStarSearch<machine_learning::aystar_search::EightPuzzle<3>>(Puzzle, Ideal);
  ...
  // Initialize the search object
  machine_learning::aystar_search::AyStarSearch<machine_learning::aystar_search::EightPuzzle<4>> search2(Puzzle2,
                                                                                                         Ideal2);
  /**
   * Heuristic function: Manhattan distance
   */
  auto manhattan_distance2 = [](const machine_learning::aystar_search::EightPuzzle<4> &first,

...
namespace machine_learning {
   ...
   */
  void update_weights(const std::valarray<double> &x, std::vector<std::valarray<double>> *W, std::valarray<double> *D,
                      double alpha, int R) {
    int j = 0, k = 0;
...
#pragma omp for
   ...
   */
  void kohonen_som_tracer(const std::vector<std::valarray<double>> &X, std::vector<std::valarray<double>> *W,
                          double alpha_min) {
...
#endif
  for (i = 0; i < N; i++) {
    int cls = std::rand() % num_classes;  // select a random class for the point

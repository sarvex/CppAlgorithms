...
namespace machine_learning {
   ...
   */
  template <typename T>
  void uniform_random_initialization(std::vector<std::valarray<T>> &A, const std::pair<size_t, size_t> &shape,
   ...
   */
  template <typename T>
  std::vector<std::vector<std::valarray<T>>> minmax_scaler(const std::vector<std::vector<std::valarray<T>>> &A,
                                                           const T &low, const T &high) {
   ...
   */
  template <typename T>
  std::vector<std::valarray<T>> hadamard_product(const std::vector<std::valarray<T>> &A,
                                                 const std::vector<std::valarray<T>> &B) {
    const auto shape_a = get_shape(A);
    const auto shape_b = get_shape(B);

...
std::vector<std::vector<T>> operator*(std::vector<std::vector<T>> const &A, std::vector<std::vector<T>> const &B) {
  ...
  if (A[0].size() != B.size()) {
    std::cerr << "Number of columns in A != Number of rows in B (" << A[0].size() << ", " << B.size() << ")"
              << std::endl;
    return result;
  }
...
void ols_test() {
  ...
  std::vector<std::vector<float>> data1({{-5, 25, -125}, {-1, 1, -1}, {0, 0, 0}, {1, 1, 1}, {6, 36, 216}});
  ...
  std::vector<std::vector<float>> test_data1({{-2, 4, -8}, {2, 4, 8}, {-10, 100, -1000}, {10, 100, 1000}});
  ...
  // create training data set with features = x, x^2, x^3
  std::vector<std::vector<float>> data2({{-5, 25, -125}, {-1, 1, -1}, {0, 0, 0}, {1, 1, 1}, {6, 36, 216}});
  // create corresponding outputs
  std::vector<float> Y2({-200, -100, -100, 98, 152});
  // perform regression modelling
  std::vector<float> beta2 = fit_OLS_regressor(data2, Y2);
  // create test data set with same features = x, x^2, x^3
  std::vector<std::vector<float>> test_data2({{-2, 4, -8}, {2, 4, 8}, {-10, 100, -1000}, {10, 100, 1000}});
  // expected regression outputs
...
int main() {
  ...
  std::cout << "Enter training data. Per sample, provide features and one output." << std::endl;

...
std::size_t longest_common_string_length(const std::string& string_a, const std::string& string_b) {
  ...
  std::vector<std::vector<std::size_t>> sub_sols(size_a + 1, std::vector<std::size_t>(size_b + 1, 0));
  ...
  for (std::size_t pos_a = size_a - 1; pos_a != limit; --pos_a) {
    for (std::size_t pos_b = size_b - 1; pos_b != limit; --pos_b) {
      if (string_a[pos_a] == string_b[pos_b]) {
        ...
        sub_sols[pos_a][pos_b] = std::max(sub_sols[pos_a + 1][pos_b], sub_sols[pos_a][pos_b + 1]);
      }
    }
  }
...
struct TestCase {
  ...
  TestCase(std::string string_a, std::string string_b, const std::size_t in_common_string_len)
    : string_a(std::move(string_a)), string_b(std::move(string_b)), common_string_len(in_common_string_len) {}
};
...
static void test_longest_common_string_length(const TestCases& test_cases) {
  for (const auto& cur_tc : test_cases) {
    assert(longest_common_string_length(cur_tc.string_a, cur_tc.string_b) == cur_tc.common_string_len);
  }
}
...
static void test_longest_common_string_length_is_symmetric(const TestCases& test_cases) {
  for (const auto& cur_tc : test_cases) {
    assert(longest_common_string_length(cur_tc.string_b, cur_tc.string_a) == cur_tc.common_string_len);
  }
}
 ...
 * in_str
 */
std::string reverse_str(const std::string& in_str) { return {in_str.rbegin(), in_str.rend()}; }
...
static void test_longest_common_string_length_for_reversed_inputs(const TestCases& test_cases) {
  for (const auto& cur_tc : test_cases) {
    assert(longest_common_string_length(reverse_str(cur_tc.string_a), reverse_str(cur_tc.string_b)) ==

  ...
  namespace reverse_binary_tree {
   ...
   private:
      ...
      Node* insert(int64_t data, Node* pivot) {
        ...
        if (data <= pivot->data) {
          pivot->left = insert(data, pivot->left);  ///< Insert Node to the left
        } else {
          pivot->right = insert(data, pivot->right);  ///< Insert node to the right
        }
        return pivot;
      }
      /**
 ...
 */
namespace tests {
  using operations_on_datastructures::reverse_binary_tree::BinaryTree;  ///< Use the BinaryTree
  /**
  ...
  void test1() {
    ...
    assert(pre_reversal.size() == post_reversal.size());  ///< Check for equal sizes
    assert(pre_reversal.size() == 1);                     ///< Ensure that there is only one element
    assert(pre_reversal[0] == post_reversal[0]);          ///< Check if both elements are same
    bst.print();
  ...
  void test2() {
    ...
    post_reversal = bst.get_level_order();
    assert(pre_reversal.size() == post_reversal.size());  ///< Check for equal sizes
    assert(pre_reversal.size() == 0);                     ///< Ensure that there is only one element
    bst.print();
    std::cout << "TEST PASSED!\n\n";
  }
  /**

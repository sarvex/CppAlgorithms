...
namespace operations_on_datastructures {
  ...
  namespace trie_operations {
   ...
   public:
      ...
      void SuggestFreqAutocomplete(Tnode *new_root, const std::string &prefix,
    ...
    void Tnode::SearchSuggestions(const std::string &key) {
      ...
      int letter_index = 0;
      std::string prefix = "";  // variable storing the updated value of longest common prefix
     ...
     */
    void Tnode::SelectionTop_3(std::priority_queue<std::pair<int, std::string> > *suggestions) {
     ...
     * heapified based on frequency
     */
    void Tnode::SuggestFreqAutocomplete(Tnode *new_root, const std::string &prefix,
                                        std::priority_queue<std::pair<int, std::string> > *suggestions) {
      int i = 0;
      for (i = 0; i < ENGLISH_ALPHABET_SIZE; i++) {
        if (new_root->english[i] != nullptr) {
          ...
          // just a prefix
          if (new_root->english[i]->endOfWord) {
            suggestions->push(std::make_pair(new_root->english[i]->frequency, prefix + char(i + 97)));
          }
    ...
    void Tnode::SearchFreqSuggestions(const std::string &key) {
      ...
      int letter_index = 0;
      std::string prefix = "";  // variable storing the updated value of longest common prefix
      std::priority_queue<std::pair<int, std::string> > suggestions;  // max heap to store (frequency, word) in
...
static void test() {
  ...
  std::vector<std::string> inputs = {"abcde", "sss",  "ssss",  "ssst",  "sssu",   "sssv",  "sst",  "ssts", "sstt",
                                     "sstu",  "tutu", "tutuv", "tutuu", "tutuvs", "tutus", "tvst", "tvsu", "vvvv"};
  ...
  root->SearchFreqSuggestions("tu");  // The top 3 frequent entries with prefix tu are tutu, tutuv &
                                      // tutuvs respectively
  root->SearchSuggestions("");        // Empty search to list all the entries in the trie
}

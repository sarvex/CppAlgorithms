...
namespace hashing {
  ...
  namespace sha1 {
     ...
     */
    uint32_t leftRotate32bits(uint32_t n, std::size_t rotate) { return (n << rotate) | (n >> (32 - rotate)); }
     ...
     */
    void* hash(const std::string& message) { return hash_bs(&message[0], message.size()); }
  }  // namespace sha1
}  // namespace hashing
...
static void test() {
  ...
  assert(hashing::sha1::sig2hex(sig).compare("da39a3ee5e6b4b0d3255bfef95601890afd80709") == 0);
  ...
  // Test with cassert wether sig is correct from expected value
  assert(hashing::sha1::sig2hex(sig2).compare("2fd4e1c67a2d28fced849ee1bb76e7391b93eb12") == 0);
  ...
  // Test with cassert wether sig is correct from expected value
  assert(hashing::sha1::sig2hex(sig3).compare("408d94384216f890ff7a0c3528e8bed1e0b01621") == 0);
  ...
  // Test with cassert wether sig is correct from expected value
  assert(hashing::sha1::sig2hex(sig4).compare("761c457bf73b14d27e9e9265c46f4b4dda11f940") == 0);
}
...
static void interactive() {
  while (true) {
    std::string input;
    std::cout << "Enter a message to be hashed (Ctrl-C to exit): " << std::endl;

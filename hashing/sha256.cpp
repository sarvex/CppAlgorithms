...
namespace hashing {
  ...
  namespace sha256 {
   ...
   public:
     ...
     */
    uint32_t right_rotate(uint32_t n, size_t rotate) { return (n >> rotate) | (n << (32 - rotate)); }
    ...
    void Hash::update(const std::array<uint32_t, 64> &blocks) {
      ...
      for (size_t block_num = 0; block_num < 64; ++block_num) {
        ...
        const auto temp1 = h + s1 + ch + round_constants[block_num] + blocks[block_num];
    ...
    char get_char(const std::string &input, std::size_t pos) {
      ...
      return static_cast<char>(extract_byte<size_t>(input_size * 8, padded_input_size - pos - 1));
    }
    ...
    std::array<uint32_t, 64> create_message_schedule_array(const std::string &input, const size_t byte_num) {
      ...
      for (size_t block_num = 0; block_num < 16; ++block_num) {
        blocks[block_num] = (static_cast<uint8_t>(get_char(input, byte_num + block_num * 4)) << 24) |
                            (static_cast<uint8_t>(get_char(input, byte_num + block_num * 4 + 1)) << 16) |
                            (static_cast<uint8_t>(get_char(input, byte_num + block_num * 4 + 2)) << 8) |
      ...
      for (size_t block_num = 16; block_num < 64; ++block_num) {
        const auto s0 = right_rotate(blocks[block_num - 15], 7) ^ right_rotate(blocks[block_num - 15], 18) ^
                        (blocks[block_num - 15] >> 3);
        const auto s1 = right_rotate(blocks[block_num - 2], 17) ^ right_rotate(blocks[block_num - 2], 19) ^
                        (blocks[block_num - 2] >> 10);
        blocks[block_num] = blocks[block_num - 16] + s0 + blocks[block_num - 7] + s1;
      }
    ...
    std::string sha256(const std::string &input) {
      ...
      for (size_t byte_num = 0; byte_num < compute_padded_size(input.length()); byte_num += 64) {
...
static void test_sha256() {
  struct TestCase {
    ...
    TestCase(std::string input, std::string expected_hash)
      : input(std::move(input)), expected_hash(std::move(expected_hash)) {}
  };

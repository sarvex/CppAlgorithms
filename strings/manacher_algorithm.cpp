  ...
  namespace manacher {
    ...
    std::string manacher(const std::string &prototype) {
      if (prototype.size() > 0) {
        ...
        uint64_t bigger_center = 0;  // this is the index of the center of palindromic
        ...
        for (uint64_t i = 1; i < stuffed_string.size() - 1; i++) {
          if (i < right) {                                   // when i is before right end, considering
            ...
            uint64_t opposite_to_i = 2 * bigger_center - i;  // this is the opposite end of string, if
            ...
            palindrome_max_half_length[i] = std::min(palindrome_max_half_length[opposite_to_i], right - i);
          }
        ...
        std::string palindromic_substring = "";  // contains the resulting largest palindrome
        ...
        if (half_length > 0) {
          ...
          uint64_t start = center_index - half_length + 1;  // index of first character of palindromic substring
          uint64_t end = center_index + half_length - 1;    // index of last character of palindromic substring

...
namespace strings {
  /**
   * @namespace horspool
   * @brief Functions for [Horspool's](https://en.wikipedia.org/wiki/Boyer%E2%80%93Moore%E2%80%93Horspool_algorithm)
   * algorithm
   */
  namespace horspool {
    /**
    ...
    bool horspool(const std::string &text, const std::string &prototype) {
      std::unordered_map<char, int> shiftTable =
        findShiftTable(prototype);  // Initialise shift table calling findShiftTable function
      ...
      while (i < text.size()) {
        ...
        if (k == prototype.size()) {  // If all characters match then return true
          ...
          if (shiftTable.find(text[i]) != shiftTable.end()) {
            ...
            i += prototype.size();  // If character does not exist in shift table then shift index as many steps as size
                                    // of prototype string
          }
        }
      }

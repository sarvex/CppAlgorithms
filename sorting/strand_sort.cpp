 ...
 *
 * @details
 * Strand Sort is a sorting algorithm that works in \f$O(n)\f$ time if list is already sorted and works in \f$O(n^2)\f$
 * in worst case.
 *
  ...
  namespace strand {
    ...
    std::list<T> strand_sort(std::list<T> lst) {
      ...
      while (!lst.empty()) /* if lst is not empty */ {
        ...
        for (auto it = lst.begin(); it != lst.end();) {  // Return the loop as long as the current iterator is not equal
                                                         // to the last literator of the "lst" list.
          if (sorted.back() <= *it) {  // If the last reference of the "sorted" list is less than or equal to the
                                       // current iterator reference.
            sorted.push_back(*it);     // Adds the iterator retrieved in the loop under the "sorted" list.
            it = lst.erase(
              it);  // Deletes the element with the current iterator and assigns the deleted element to the iterator.
          } else {
            it++;  // Next iterator.
          }
        }

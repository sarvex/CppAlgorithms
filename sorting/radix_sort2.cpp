...
namespace sorting {
  ...
  namespace radix_sort {
     ...
     */
    std::vector<uint64_t> step_ith(uint16_t cur_digit,
      ...
      for (int i = 0; i < n; ++i) {
        temp[position[(ar[i] / cur_digit) % 10]] = ar[i];  // storing ar[i] in ar[i]'s cur_digit expected position of
                                                           // this step.
        position[(ar[i] / cur_digit) % 10]++;              // incrementing ar[i]'s cur_digit position by 1, as
    ...
    std::vector<uint64_t> radix(const std::vector<uint64_t>& ar) {
      ...
      for (int i = 1; max_ele / i > 0; i *= 10) {  // loop breaks when i > max_ele because no further digits
...
static void tests() {
  ...
  /// Test 2
  std::vector<uint64_t> ar2 = {213, 3214, 123, 111, 112, 142, 133, 132, 32, 12, 113};

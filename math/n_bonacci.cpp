...
namespace math {
  ...
  namespace n_bonacci {
    ...
    std::vector<uint64_t> N_bonacci(const uint64_t &n, const uint64_t &m) {
      std::vector<uint64_t> a(m, 0);  // we create an array of size m filled with zeros
      if (m < n || n == 0) {
        return a;
      }
...
static void test() {
  struct TestCase {
    ...
    TestCase(const uint64_t in_n, const uint64_t in_m, std::initializer_list<uint64_t> data)
  ...
  const std::vector<TestCase> test_cases = {
    ...
    TestCase(6, 17, {0, 0, 0, 0, 0, 1, 1, 2, 4, 8, 16, 32, 63, 125, 248, 492, 976}),

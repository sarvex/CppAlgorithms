...
bool test1() {
  ...
  if (strlen(known_reslt) != result.num_digits()) {
    std::cerr << "Result lengths dont match! " << strlen(known_reslt) << " != " << result.num_digits() << std::endl;
    return false;
  }
  ...
  for (i = 0; i < N; i++) {
    if (known_reslt[i] != result.digit_char(i)) {
      std::cerr << i << "^th digit mismatch! " << known_reslt[i] << " != " << result.digit_char(i) << std::endl;
      return false;
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  std::cout << number << "! = " << result << std::endl
            ...
            << "Time taken: " << std::scientific << time_taken << " s" << std::endl;

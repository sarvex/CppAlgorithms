...
namespace math {
  ...
  namespace monte_carlo {
     ...
     * the pdf
     */
    std::vector<double> generate_samples(const double& start_point, const Function& pdf, const uint32_t& num_samples,
     ...
     * / p(x_i)
     */
    double integral_monte_carlo(const double& start_point, const Function& function, const Function& pdf,
...
static void test() {
  ...
  */
  ...
  pdf = [&](double& x) { return 1.0 / std::sqrt(2.0 * M_PI) * std::exp(-x * x / 2.0); };

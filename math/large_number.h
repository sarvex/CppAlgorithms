...
class large_number {
  ...
  inline const unsigned char &operator[](size_t n) const { return this->_digits[n]; }

  /**
   * operator overload to compare two numbers
   **/
  friend std::ostream &operator<<(std::ostream &out, const large_number &a) {
    for (size_t i = a.num_digits(); i > 0; i--) out << static_cast<int>(a[i - 1]);
    return out;
  }
   ...
   * operator overload to compare two numbers
   **/
  friend bool operator!=(large_number const &a, large_number const &b) { return !(a == b); }
   ...
   **/
  char digit_char(size_t i) const { return _digits[num_digits() - i - 1] + '0'; }

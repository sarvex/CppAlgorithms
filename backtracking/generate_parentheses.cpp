/**
 * @brief Well-formed [Generated Parentheses](https://leetcode.com/explore/interview/card/top-interview-questions-medium/109/backtracking/794/) with all combinations.
 * @details a sequence of parentheses is well-formed if each opening parentheses has a corresponding closing parenthesis and the closing parentheses are correctly ordered
 */
...
namespace backtracking {
  ...
  void generate_parentheses::makeStrings(std::string str, int n, int closed, int open) {
    ...
    if ((str.length() == 2 * n) && (closed != open)) {  // closed and open must be the same
      return;
    }

 ...
 */
  ...
  bool wildcard_matching(std::string s, std::string p, uint32_t pos1, uint32_t pos2) {
    uint32_t n = s.length();
    uint32_t m = p.length();
    ...
    if (s[pos1] == p[pos2]) {
      ...
      // have choice either to match one or more charcters
        ...
        return dpTable[pos1][pos2] = wildcard_matching(s, p, pos1, pos2 + 1) || wildcard_matching(s, p, pos1 + 1, pos2);
      }
...
}}  // namespace backtracking::wildcard_matching
...
assert(backtracking::wildcard_matching::wildcard_matching(matching1, pattern1, 0, 0) ==
...
assert(backtracking::wildcard_matching::wildcard_matching(matching2, pattern2, 0, 0) ==
       1);  // here the pattern matches with given string
std::cout << "passed" << std::endl;
...
assert(backtracking::wildcard_matching::wildcard_matching(matching3, pattern3, 0, 0) ==
       1);  // here the pattern matches with given string
std::cout << "passed" << std::endl;
...
assert(backtracking::wildcard_matching::wildcard_matching(matching4, pattern4, 0, 0) ==
       1);  // here the pattern matches with given string
std::cout << "passed" << std::endl;
...
assert(backtracking::wildcard_matching::wildcard_matching(matching5, pattern5, 0, 0) ==
       1);  // here the pattern matches with given string
std::cout << "passed" << std::endl;
}

 ...
 * @details A knight's tour is a sequence of moves of a knight on a chessboard such that the knight visits every square only once. If the knight ends on a square that is one knight's move from the beginning square (so that it could tour the board again immediately, following the same path, the tour is closed; otherwise, it is open.
 */
#include <array>     /// for std::array
#include <iostream>  /// for IO operations

namespace backtracking { namespace knight_tour {
  /**
  ...
  /**
     ...
     */
  template <size_t V>
  bool solve(int x, int y, int mov, std::array<std::array<int, V>, V>& sol, const std::array<int, V>& xmov,
...
}}  // namespace backtracking::knight_tour

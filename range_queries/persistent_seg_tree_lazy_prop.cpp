...
namespace range_queries {
 ...
 private:
   ...
   public:
    ...
    std::vector<std::shared_ptr<Node>> ptrs{};  /// ptrs[i] holds a root pointer to the segment tree after the
     ...
     */
    std::shared_ptr<Node> update(const uint32_t &i, const uint32_t &j, const uint32_t &l, const uint32_t &r,
                                 const int64_t &value, std::shared_ptr<Node> const &curr) {
      lazy(i, j, curr);
      if (i >= l && j <= r) {
     ...
     */
    int64_t query(const uint32_t &i, const uint32_t &j, const uint32_t &l, const uint32_t &r,
 ...
 public:
     ...
     * @returns void
     */
    void construct(const std::vector<int64_t> &vec)  // the segment tree will be built from the values
    ...
    void update(const uint32_t &l, const uint32_t &r,
                const int64_t &value)  // all elements from index "l" to index "r" would
     ...
     * @returns sum of elements whose index x satisfies l<=x<=r
     */
    int64_t query(const uint32_t &l, const uint32_t &r,
                  const uint32_t &version)  // querying the range from "l" to "r" in a segment tree

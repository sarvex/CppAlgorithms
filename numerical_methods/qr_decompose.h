...
namespace qr_algorithm {
  ...
  std::ostream &operator<<(std::ostream &out, std::valarray<std::valarray<T>> const &v) {
    ...
    for (size_t row = 0; row < v.size(); row++) {
      for (size_t col = 0; col < v[row].size(); col++)
        out << std::right << std::setw(width) << std::setfill(separator) << v[row][col];
      out << std::endl;
    }
  ...
  std::ostream &operator<<(std::ostream &out, std::valarray<T> const &v) {
    ...
    out.precision(4);
    for (size_t row = 0; row < v.size(); row++) {
      out << std::right << std::setw(width) << std::setfill(separator) << v[row];
    }
   ...
   */
  template <typename T>
  void qr_decompose(const std::valarray<std::valarray<T>> &A, /**< input matrix to decompose */

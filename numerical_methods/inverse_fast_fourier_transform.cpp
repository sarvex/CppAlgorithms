...
namespace numerical_methods {
  ...
  std::complex<double> *InverseFastFourierTransform(std::complex<double> *p, uint8_t n) {
    ...
    std::complex<double> *ye = InverseFastFourierTransform(pe, n / 2);  /// Recursive Call

    std::complex<double> *yo = InverseFastFourierTransform(po, n / 2);  /// Recursive Call
...
static void test() {
  ...
  uint8_t n1 = 2;
  uint8_t n2 = 4;
  std::vector<std::complex<double>> r1 = {{1, 0}, {2, 0}};  /// True Answer for test case 1

  std::vector<std::complex<double>> r2 = {{1, 0}, {2, 0}, {3, 0}, {4, 0}};  /// True Answer for test case 2
  ...
  for (uint8_t i = 0; i < n1; i++) {
    assert((r1[i].real() - o1[i].real() < 0.000000000001) &&
           (r1[i].imag() - o1[i].imag() < 0.000000000001));  /// Comparing for both real and imaginary
                                                             /// values for test case 1
  }

  for (uint8_t i = 0; i < n2; i++) {
    assert((r2[i].real() - o2[i].real() < 0.000000000001) &&
           (r2[i].imag() - o2[i].imag() < 0.000000000001));  /// Comparing for both real and imaginary

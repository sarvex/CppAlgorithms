...
#pragma parallel omp for private(i)
      ...
      for (i = 0; i < L2 - key_len + 1; i += key_len) {
        ...
        for (size_t j = 0; j < key_len; j++) {
          coded_text[i + j] = STRKEY[batch_int[j]];  // get character at key
        }
      }

/**
 @brief [Base64 Encoding and Decoding](https://en.wikipedia.org/wiki/Base64)
 @details In programming, [Base64](https://en.wikipedia.org/wiki/Base64) is a group of binary-to-text encoding schemes that represent binary data (more specifically, a sequence of 8-bit bytes) in an ASCII string format by translating the data into a radix-64 representation. The term Base64 originates from a specific MIME content transfer encoding. Each non-final Base64 digit represents exactly 6 bits of data. Three 8-bit bytes (i.e., a total of 24 bits) can therefore be represented by four 6-bit Base64 digits.
 */
#include <array>     /// for `std::array`
#include <cassert>   /// for `assert` operations
#include <iostream>  /// for IO operations

namespace ciphers { namespace base64_encoding {
 ...
 */
  std::string base64_decode(const std::string& base64_str) {
    std::string base64_decoded;  /// Output of this function: base64 decoded string
    for (uint32_t i = 0; i < base64_str.size(); i += 4) {
      ...
      if (i + 2 < base64_str.size() && base64_str[i + 2] != '=') {
        ...
        char second_actual_byte =
          static_cast<char>(((find_idx(second_byte) & 0x0F) << 4) | (find_idx(third_byte) >> 2));
...
}}  // namespace ciphers::base64_encoding

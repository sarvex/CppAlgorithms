/**
 ...
 @details In cryptography, the simple XOR cipher is a type of additive cipher, an encryption algorithm that operates according to the principles: \f$A {\oplus} 0 = A\f$ \f$A {\oplus} A = 0\f$ \f$ (A {\oplus} B) {\oplus} C = A {\oplus} (B {\oplus} C)\f$ \f$ (B {\oplus} A) {\oplus} B = B {\oplus} 0 = B \f$ where \f$\oplus\f$ symbol denotes the exclusive disjunction (XOR) operation. This operation is sometimes called modulus 2 addition (or subtraction, which is identical). With this logic, a string of text can be encrypted by applying the bitwise XOR operator to every character using a given key. To decrypt the output, merely reapplying the XOR function with the key will remove the cipher. Choose the key for encryption and apply XOR operation to each character of a string. Reapplying XOR operation to each character of encrypted string will give original string back.
 @note This program implements XOR Cipher for string with ASCII characters.
 */
#include <cassert>
#include <iostream>
#include <string>

namespace ciphers { namespace XOR {
  /**

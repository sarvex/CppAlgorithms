...
std::string add(const std::string &first, const std::string &second) {
  ...
  for (int32_t i = static_cast<int32_t>(first.size()) - 1, j = static_cast<int32_t>(second.size()) - 1;
       i >= 0 || j >= 0; --i, --j) {
...
class uint128_t {
   ...
   */
  explicit uint128_t(const std::string &str) { __get_integer_from_string(str); }
   ...
   * @returns addition of this and p, returning uint128_t integer
   */
  inline uint128_t operator+(const uint128_t &p) { return uint128_t(f + (p.s + s < s) + p.f, p.s + s); }
   ...
   */
  uint128_t operator*(const uint128_t &p) {
    uint64_t f_first = s >> 32, f_second = s & 0xFFFFFFFF, s_first = p.s >> 32, s_second = p.s & 0xFFFFFFFF;
    uint64_t fi = f_first * s_first, se = f_first * s_second, th = s_first * f_second, fo = s_second * f_second;
    uint64_t tmp = ((se & 0xFFFFFFFF) << 32), tmp2 = (th & 0xFFFFFFFF) << 32;
    int cc = (tmp + tmp2 < tmp);
    tmp += tmp2;
    cc += (tmp + fo < tmp);
   ...
   */
  inline bool operator<(const uint128_t &other) { return f < other.f || (f == other.f && s < other.s); }
   ...
   */
  inline bool operator==(const uint128_t &other) { return f == other.f && s == other.s; }
   ...
   */
  inline bool operator&&(const uint128_t &b) { return (s || f) && (b.s || b.f); }
   ...
   */
  inline uint128_t operator&(const uint128_t &p) { return uint128_t(this->f & p.f, this->s & p.s); }

...
namespace physics {
  ...
  namespace ground_to_ground_projectile_motion {
     ...
     */
    double degrees_to_radians(double radian, double PI = 3.14) { return (radian * (PI / 180)); }
    ...
    T time_of_flight(T initial_velocity, T angle, double gravity = 9.81) {
      double Viy =
        initial_velocity * (std::sin(degrees_to_radians(angle)));  // calculate y component of the initial velocity
    ...
    T horizontal_range(T initial_velocity, T angle, T time) {
      double Vix =
        initial_velocity * (std::cos(degrees_to_radians(angle)));  // calculate x component of the initial velocity
      return Vix * time;
    }

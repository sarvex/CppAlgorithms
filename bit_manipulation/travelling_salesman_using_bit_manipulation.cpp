/**
 @brief Implementation to [Travelling Salesman problem using bit-masking] (https://www.geeksforgeeks.org/travelling-salesman-problem-set-1/)
 @details Given the distance/cost(as and adjacency matrix) between each city/node to the other city/node , the problem is to find the shortest possible route that visits every city exactly once and returns to the starting point or we can say the minimum cost of whole tour.
...
namespace bit_manipulation { namespace travelling_salesman_using_bit_manipulation {
  /**
 ...
 */
  std::uint64_t travelling_salesman_using_bit_manipulation(
    std::vector<std::vector<uint32_t>> dist,  // dist is the adjacency matrix containing the distance.
...
}}  // namespace bit_manipulation::travelling_salesman_using_bit_manipulation
...
static void test() {
// 1st test-case
std::vector<std::vector<uint32_t>> dist = {{0, 20, 42, 35}, {20, 0, 30, 34}, {42, 30, 0, 12}, {35, 34, 12, 0}};
uint32_t V = dist.size();
std::vector<std::vector<uint32_t>> dp(1 << V, std::vector<uint32_t>(V, -1));
assert(bit_manipulation::travelling_salesman_using_bit_manipulation::travelling_salesman_using_bit_manipulation(
         dist, 1, 0, V, dp) == 97);

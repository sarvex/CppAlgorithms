...
class Node {
   ...
   */
  explicit Node(int64_t item) : count(1), items({{item, 0, 0}}), children({{nullptr, nullptr, nullptr, nullptr}}) {}
 ...
 * nodes
 */
void Tree234::PrintNode(std::ofstream &ofs, Node *node, int64_t parent_index, int64_t index,
  ...
  switch (node->GetCount()) {
    ...
    case 2:
      ...
      break;
    case 3:
      ofs << "node_" << index << " [label=\"<f0> " << node->GetItem(0) << " | <f1> " << node->GetItem(1) << "| <f2> "
          << node->GetItem(2) << "\"]\n";
      break;
  ...
  // draw the edge
  if (parent_index >= 0) {
    ofs << "node_" << parent_index << ":f" << (parent_child_index == 0 ? 0 : parent_child_index - 1) << ":"

...
class Trie {
   ...
   */
  struct Node {
    std::unordered_map<char16_t, std::shared_ptr<Node>> children;  ///< unordered map with key type char16_t and value
                                                                   ///< is a shared pointer type of Node
    bool word_end = false;  ///< boolean variable to represent the node end
  };

  std::shared_ptr<Node> root_node = std::make_shared<Node>();  ///< declaring root node of trie
   ...
   */
  std::vector<std::string> get_all_words(std::vector<std::string> results, const std::shared_ptr<Node>& element,
                                         std::string prefix) {
    if (element->word_end) {
  ...
  std::vector<std::string> predict_words(const std::string& prefix) {
    ...
    result = get_all_words(result, curr,

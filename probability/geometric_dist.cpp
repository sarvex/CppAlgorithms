...
namespace probability {
  ...
  namespace geometric_dist {
     ...
     * (included)
     */
    float generate_uniform() { return static_cast<float>(rand()) / static_cast<float>(RAND_MAX); }
   ...
   public:
       ...
       */
      float probability_density(const uint32_t& k) const { return std::pow((1.0f - p), static_cast<float>(k - 1)) * p; }

...
#pragma omp for
  ...
  void test() {
    ...
    fname << std::setw(3) << "spirograph_" << l << "_" << k << "_" << rot << ".csv";
   ...
   */
  template <size_t N>
  void display_graph(const std::array<std::pair<double, double>, N> &points, double l, double k) {
    ...
    glRasterPos2f(-.85, .85);
    glutBitmapString(GLUT_BITMAP_TIMES_ROMAN_24, const_cast<char *>(buffer.str().c_str()));
    buffer.str("");
    buffer.clear();

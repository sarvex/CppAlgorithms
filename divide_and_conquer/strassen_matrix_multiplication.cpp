...
class Matrix {
   ...
   */
  inline std::pair<size_t, size_t> size() const { return {_mat.size(), _mat[0].size()}; }
   ...
   */
  Matrix slice(const size_t row_start, const size_t row_end = MAX_SIZE, const size_t col_start = MAX_SIZE,
  ...
  template <typename Number, typename = typename std::enable_if<
                               std::is_integral<Number>::value || std::is_floating_point<Number>::value, Number>::type>
   ...
   */
  template <typename Number, typename = typename std::enable_if<
                               std::is_integral<Number>::value || std::is_floating_point<Number>::value, bool>::type>
  ...
  Matrix strassens_multiplication(const Matrix<Number> &other) const {
    ...
    if (size <= 64ULL || (size & 1ULL)) {
      ...
      const Matrix<Number> A = this->slice(0ULL, size >> 1, 0ULL, size >> 1),
  ...
  bool operator==(const Matrix<T> &other) const {
    if (_mat.size() != other._mat.size() || _mat[0].size() != other._mat[0].size()) {
      return false;
    }
    for (size_t i = 0; i < _mat.size(); ++i) {
      for (size_t j = 0; j < _mat[i].size(); ++j) {
...
static void test() {
  const size_t s = 512;
  auto matrix_demo = divide_and_conquer::strassens_multiplication::Matrix<size_t>(s, s);
  ...
  auto matrix_demo2 = divide_and_conquer::strassens_multiplication::Matrix<size_t>(s, s);
  for (size_t i = 0; i < s; ++i) {
    for (size_t j = 0; j < s; ++j) {

...
std::string add_strings(std::string first, std::string second) {
  ...
  for (int64_t i = length - 1; i >= 0; i--) {
    ...
    carry = char((firstBit & secondBit) | (secondBit & carry) | (firstBit & carry));  // sum of 3 bits
  }
...
int64_t karatsuba_algorithm(std::string str1, std::string str2) {
  ...
  std::string Xr =
    divide_and_conquer::karatsuba_algorithm::safe_substr(str1, fh, sh, n);  // second half of first string

  std::string Yl = divide_and_conquer::karatsuba_algorithm::safe_substr(str2, 0, fh, n);  // first half of second string
  std::string Yr =
    divide_and_conquer::karatsuba_algorithm::safe_substr(str2, fh, sh, n);  // second half of second string
  ...
  int64_t product3 = karatsuba_algorithm(divide_and_conquer::karatsuba_algorithm::add_strings(Xl, Xr),
                                         divide_and_conquer::karatsuba_algorithm::add_strings(Yl, Yr));

  return product1 * (1 << (2 * sh)) + (product3 - product1 - product2) * (1 << sh) +
